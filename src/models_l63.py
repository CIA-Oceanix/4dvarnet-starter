...
class Lit4dVarNet_L63(pl.LightningModule):
    def __init__(self,params=None,patch_weight=None,stats_training_data=None,*args, **kwargs):
        ...
        if self.hparams.solver =='4dvarnet-with-rnd' :
            self.model        = solver_4DVarNet.GradSolver_with_rnd(Phi, 
                                                                    ...
                                                                    None, None, 
                                                                    self.hparams.shapeData, self.hparams.n_grad, EPS_NORM_GRAD,self.hparams.k_n_grad,self.hparams.lr_grad,self.hparams.lr_rnd)#, self.hparams.eps_norm_grad)            
    ...
    def loss_from_perturbation(self,x,y,mask,phase):
        ...
        if self.hparams.degradation_operator == 'no-degradation' :
            ...
            print('%f -- %f --  %f -- %f'%(torch.mean( dx**2 ).detach().cpu().numpy(),torch.mean( var_cost_grad**2 ).detach().cpu().numpy(),loss.detach().cpu().numpy(), torch.nanmean( dx * var_cost_grad / ( n_dx * n_grad ) ).detach().cpu().numpy() ) )
