...
class Model_H3(torch.nn.Module):
    def __init__(self,shape_data,dim=5,sampling=3,padding_mode='reflect'):
        super(Model_H3, self).__init__()
        ...
        self.bn_x_feat = torch.nn.BatchNorm1d(self.dimObsChannel[0],track_running_stats=False)
        self.bn_y_feat = torch.nn.BatchNorm1d(self.dimObsChannel[0],track_running_stats=False)

        self.poolx   = torch.nn.AvgPool2d((self.sampling,1))
        self.convx11 = torch.nn.Conv2d(shape_data[0], 2*self.dimObsChannel[0], (2*int(self.sampling/2)+1, 1), padding=(int(self.sampling/2),0), bias=False,padding_mode=padding_mode)
...
class Model_H4(torch.nn.Module):
    def __init__(self,shape_data,dim=5,sampling=3,padding_mode='reflect'):
        super(Model_H4, self).__init__()
        ...
        #self.convx11 = torch.nn.Conv2d(shape_data[0], 2*self.dimObsChannel[0], (2*int(self.sampling/2)+1, 1), padding=(int(self.sampling/2),0), bias=False,padding_mode=padding_mode)
        #self.convx12 = torch.nn.Conv2d(2*self.dimObsChannel[0], self.dimObsChannel[0], (3, 1), padding=(1,0), bias=False,padding_mode=padding_mode)
        self.fcx     = torch.nn.Linear(shape_data[0]*shape_data[1],self.dimObsChannel[0])
        
        #self.convx21 = torch.nn.Conv2d(self.dimObsChannel[0], 2*self.dimObsChannel[0], (3, 1), padding=(1,0), bias=False,padding_mode=padding_mode)
        #self.convx22 = torch.nn.Conv2d(2*self.dimObsChannel[0], self.dimObsChannel[0], (3, 1), padding=(1,0), bias=False,padding_mode=padding_mode)

        self.fcy1     = torch.nn.Linear(int(3*dT/self.sampling),self.dimObsChannel[0])
        #self.fcy2     = torch.nn.Linear(4*self.dimObsChannel[0],self.dimObsChannel[0])
    ...
    def extract_state_feature(self,x):
        ...
        x_feat = self.fcx( x1 )
        #x_feat = self.bn_x_feat( self.fcx( x1 ) )
    ...
    def extract_obs_feature(self,y):
        ...
        #y_feat = self.bn_y_feat( self.fcy1(y))
        y_feat = self.fcy1(y)
...
class Model_H5(torch.nn.Module):
    def __init__(self,shape_data,dim=5,sampling=3,padding_mode='reflect'):
    ...
    def compute_loss(self, batch, phase, batch_init = None , hidden = None , cell = None , normgrad = 0.0,prev_iter=0):
        with torch.set_grad_enabled(True):
            ...
            if self.hparams.use_rk4_gpu_as_target :
                ...
                print( self.ode_solver._odeL63(y0.view(1,-1,1)).detach().cpu().numpy() )
                ...
                for kk in range(0,3):
                    print('--')
                    print(x_pred[0,kk,:3].detach().cpu().numpy().transpose().squeeze() * self.stdTr + self.meanTr)
                    #print(y0.detach().cpu().numpy().transpose() )
                    print(y_ode[kk,1:4].squeeze() )
                print('....')
