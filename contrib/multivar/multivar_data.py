...
class MultivarXrDataset(XrDatasetMovingPatchFastRecGPU):
    ...
    def reconstruct_from_items(self, items: torch.Tensor, weight=None, leadtime=0):
        ...
        patch_dims_leadtime = self.patch_dims
        patch_dims_leadtime['time'] = 1
        coords_dims = patch_dims_leadtime
        ...
        for idx, coord_slices in enumerate(coords_slices):
            coord_slices['time'] = slice(coord_slices['time'].start + leadtime, coord_slices['time'].start + leadtime + time_cut)
