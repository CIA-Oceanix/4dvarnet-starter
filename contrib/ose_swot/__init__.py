...
class XrDatasetMovingPatchFastRecGPU_SWOT(XrDatasetMovingPatch):
    ...
    def __getitem__(self, item):
        ...
        # Step 1: Check if there is SWOT data for this patch
        if hasattr(self, 'swot_mask'):
            cycles_with_data = self.get_cycles_for_time(time_indices)

            if not cycles_with_data:
                return None  # or skip, raise IndexError, etc.

            # You can choose the first one, or randomly pick if multiple cycles overlap
            selected_cycle = cycles_with_data[0]

            # Step 2: Open the corresponding SWOT data file
            swot_file = f"{self.swot_data_dir}/cycle_{selected_cycle:03d}_gridded.nc"

            try:
                swot_ds = xr.open_dataset(swot_file)
            except FileNotFoundError:
                print(f"Missing SWOT file: {swot_file}")
                return None
...
def open_glorys12_data_sla_OSE_SWOT(path, masks_path, real_traces, domain, variables="sla", masking=True, test_cut=None): # zos before
    """
