...
TrainingItem_wcov = namedtuple(
    'TrainingItem_wcov', ['input', 'tgt', 'u', 'v']
)
...
class XrDataset_wcov(XrDataset):
    def __getitem__(self, item):
        sl = {
                dim: slice(self.strides.get(dim, 1) * idx,
                           self.strides.get(dim, 1) * idx + self.patch_dims[dim])
...
class BaseDataModule(pl.LightningDataModule):
    def __init__(self, input_da, domains, xrds_kw, dl_kw, aug_kw=None, resize_factor=1, res=0.05, norm_stats=None, frcst_lead=None,**kwargs):
        super().__init__()
...
class BaseDataModule_wcov(BaseDataModule):

    def norm_stats(self, variable="tgt"):
        if self._norm_stats is None:
            stats = self.train_mean_std(variable=variable)
            print("Norm stats", self.stats)
        else:
            stats = self._norm_stats[variable]
        return stats

    def post_fn(self):
        m , s = self.norm_stats()[0], self.norm_stats()[1]
        mu, su = self.norm_stats("u")[0], self.norm_stats("u")[1]
        mv, sv = self.norm_stats("v")[0], self.norm_stats("v")[1]
        normalize = lambda item: (item - m) / s
        normalize_u = lambda item: (item - mu) / su
        normalize_v = lambda item: (item - mv) / sv
        return ft.partial(
            ft.reduce,
            lambda i, f: f(i),
            [
                TrainingItem_wcov._make,
                lambda item: item._replace(input=normalize(item.input)),
                lambda item: item._replace(tgt=normalize(item.tgt)),
                lambda item: item._replace(u=normalize_u(item.u)),
                lambda item: item._replace(v=normalize_v(item.v)),
            ],
        )
    ...
    def setup(self, stage='test'):
        ...
        self.train_ds = XrDataset_wcov(
            train_data, **self.xrds_kw, postpro_fn=post_fn,
            resize_factor = self.resize_factor, frcst_lead = self.frcst_lead,
            res = self.res
        )
        if self.aug_kw:
            self.train_ds = AugmentedDataset(self.train_ds, **self.aug_kw)
