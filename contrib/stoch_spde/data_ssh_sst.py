...
TrainingItem_ssh_sst = namedtuple(
    'TrainingItem_ssh_sst', ['input', 'tgt',
                     'input_sst', 'tgt_sst']
)
    ...
    def reconstruct_from_items(self, items, weight=None, crop=None):
        ...
        if crop is None:
            ...
            if self.frcst_lead is None:
                coords = [item.isel(time=slice(crop//4,-crop//4)) for item in self.get_coords()]
            else:
                coords = [item.isel(time=slice(crop//2,
...
class BaseDataModule(pl.LightningDataModule):
    ...
    def norm_stats(self):
        if self._norm_stats is None:
            self._norm_stats = [ self.train_mean_std('tgt_ssh')[0],
                                 self.train_mean_std('tgt_ssh')[1],
                                 self.train_mean_std('tgt_sst')[0],
                                 self.train_mean_std('tgt_sst')[1]
                               ]
            print("Norm stats", self._norm_stats)
        return self._norm_stats

    def train_mean_std(self, variable='tgt_ssh'):
    ...
    def post_fn(self):
        ...
        normalize_ssh = lambda item: (item - m_ssh) / s_ssh
        normalize_sst = lambda item: (item - m_sst) / s_sst
        update = ft.partial(ft.reduce,lambda i, f: f(i), [
            TrainingItem_ssh_sst._make,
            lambda item: item._replace(input=np.concatenate((normalize_ssh(item.input),
                                                        normalize_sst(item.input_sst)),axis=0)),
            lambda item: item._replace(tgt=np.concatenate((normalize_ssh(item.tgt),
                                                      normalize_sst(item.tgt_sst)),axis=0)),
